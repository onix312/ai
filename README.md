...
- **Помощник NOZZA** — `assistant.html` (он же `/assist` и `/помощник`): разговор с локальной
  моделью, каталог действий панели, голос и входящее сообщение с файлом; на компьютере
  открывается своим окном — `python pf.py assistant`. Инструкция: [`docs/ПОМОЩНИК.md`](docs/ПОМОЩНИК.md).
...
├── agent/                          # внешний агент компьютера: голос, стоп-слово, окна (свои зависимости)
├── connector/
