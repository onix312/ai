...
| --- | --- |
| 18.13 | AMS-автопилот: катушка узнаётся по метке (а без метки — по истории слотов и цвету), переносится в слот вместо «снял и завёл», пустая уходит на склад; настройки слота (тип, цвет, температуры) приводятся к складу и откатываются из журнала; AMS-доктор с планом раскладки и правилами, предполёт перед печатью |
