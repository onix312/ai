...
- **Mini App цеха** — кнопка «🏭 Открыть цех» в боте сотрудников: панель, полка, касса и очередь внутри Telegram. Нужен внешний HTTPS-адрес (Tailscale или туннель) — настройка по шагам: [`docs/MINIAPP-ЦЕХА.md`](docs/MINIAPP-ЦЕХА.md).
- **Бот и без Mini App** — пока адреса нет, бот отвечает сам: `статус`, `принтеры`, `заказы [номер]`, `полка`, `очередь`, `кадр`, `деньги` (деньги — владельцу и руководителю). Фото приходит перед текстом: кадр камеры или снимок изделия, остатки и очередь — списком.
...
| [`docs/MINIAPP-ЦЕХА.md`](docs/MINIAPP-ЦЕХА.md) | Mini App цеха: HTTPS-адрес, кнопка в боте, доступ сотрудникам, разбор ошибок |
