...
- **Личный ассистент компьютера** — `assistant.html` (он же `/assist` и `/помощник`): разговор с локальной
  моделью, каталог действий панели, ответ по фактам базы, день цеха, навыки файлов и знаний; своя база и своё окно на порту агента (`/ui`), на компьютере
  открывается своим окном — `python pf.py assistant --local` или двойным кликом по `ЗАПУСТИТЬ-ПОМОЩНИК.bat` (Windows). Реестр навыков: [`docs/НАВЫКИ.md`](docs/НАВЫКИ.md). Инструкция: [`docs/ПОМОЩНИК.md`](docs/ПОМОЩНИК.md).
...
- **AMS** — слоты из телеметрии и память базы: автопривязка RFID и однозначных
  катушек без RFID, переезды, долив/потеря, журнал с безопасным откатом; доктор
  AMS, проверка пластика до печати и только советующий план раскладки. Запись
  складских настроек идёт лишь на свободном принтере; один набор не чаще раза
  в 30 минут. Выключатель `ams_push_settings` запрещает все команды записи;
...
- **Бот сотрудников — всё кнопками** — меню на callback-кнопках: `статус`, `принтеры`, `заказы [номер]`, `полка`, `очередь`, `кадр`, `деньги` (деньги — владельцу и руководителю); кнопки в уведомлениях («Кадр», «Следующее», «Снял», «Продолжить») отвечают в чате. Фото приходит перед текстом: кадр камеры или снимок изделия, остатки и очередь — списком. Внешний адрес боту не нужен.
- **Ассистент в боте** — «🤖 Ассистент» (владельцу и руководителю): вопрос словами — «что печатает P1S», «кто нам должен», «сколько заказов в работе» — отвечает тот же мозг помощника, что и в панели, с памятью разговора. Действия с деньгами и печатью из чата не выполняются — только предлагаются и подтверждаются в панели.
...
| [`docs/ОТЧЁТ-18.12.3.md`](docs/ОТЧЁТ-18.12.3.md) | Отчёт 18.12.3: отчёты бота текстом и картинками, что нашлось и починено |
| [`docs/ТЗ-18.0-ПУЛЬТ-ЦЕХА.md`](docs/ТЗ-18.0-ПУЛЬТ-ЦЕХА.md) | Зачем пульт, что в каждом этапе, план коммитов |
| [`docs/ПОМОЩНИК.md`](docs/ПОМОЩНИК.md) | Локальный помощник: как включить модель, голос и агента; чеки-лист ручной проверки |
...
| [`docs/ШЛЮЗ-BAMBU-STUDIO.md`](docs/ШЛЮЗ-BAMBU-STUDIO.md) | Шлюз Bambu Studio: адрес, Access Code, порты, диагностика «код=-1» |
...
| [`ПРОМТ-ГРИЛЛ.md`](ПРОМТ-ГРИЛЛ.md), [`ПРОМТ-ПЕРЕСМОТР-СИСТЕМ.md`](ПРОМТ-ПЕРЕСМОТР-СИСТЕМ.md) | Промты-допросы: согласовать план до кода и разобрать систему |

<a id="текущая-версия"></a>

## Текущая версия: 18.21.0
...
| --- | --- |
| 18.21 | Мозг помощника: одна точка разговора, память «запомни/забудь», контекст («а у второго?», «поставь его на паузу»), станок и заказ по имени, уточнение вместо догадки, руки на ПК (звук, окна, программы, питание, озвучка), агент закрыт от сайтов, новая страница помощника |
| 18.20 | Мозги помощника: дата и время из часов без веб-поиска, смешанные вопросы «что печатаем и из какого пластика» — живое задание + остаток его материала, «что сейчас печатается» детерминированно, дайджест фактов цеха (парк, склад, долги) в промпте разговора, санация выдержек поиска. Экспорт AMS с вкладки «Принтеры» (`POST /api/ams/export`: цвета и настройки привязанных катушек, занятый принтер — отложено) и мультиколор в плане AMS (явные катушки заказа, список цветов, деление граммов) |
| 18.18 | Автоустановка зависимостей и моделей: system.check/install, pip только недостающее, vosk small ru, tesseract/ollama проверка (И222) |
| 18.19 | Надзор за процессами: пульс агента и панели, подъём упавших, эскалация после порога, `pf.py watchdog`, проверка встроенного JS в check.py, починены `system.check/install` из 18.18 и встроенный скрипт панели помощника (И263) |
| 18.17 | Полноценный ассистент ПК: голос+система+зрение+окна, автозагрузка, процессы, звук, буфер, фокус-таймер, слежка за папками, быстрый поиск, предпочтения, whitelist, макросы (И206,И208,И209,И211-И215,И217-И221) |
| 18.16 | Архив Авито, связка с заказом, расписание, дедуп, календарь ТГ, шаблоны, хештеги, FTS, экспорт, статистика |
| 18.15 | Авито-слежка и ТГ-посты: варианты ответа и публикация с подтверждением |
| 18.14 | Личный ассистент компьютера: реестр навыков вместо веток, своя SQLite (память, индекс, журнал, факты, заметки, выученные навыки), чтение файлов без зависимостей (.docx/.xlsx как zip, .gcode комментарии, .stl метаданные), план раскладки загрузок, ответ по фактам базы и день цеха без модели, окно ассистента `/ui` |
| 18.13 | Локальный помощник: своя панель и окно, каталог из 20 действий, входящее сообщение с файлом в один черновик, голос на процессоре, агент компьютера со стоп-словом и подтверждением |
| 18.12 | GPU-сечения: предпросмотр нарезки в браузере (WebGPU или процессор); регулятор, тракт AMS и шкала слоёв во вкладке «Принтеры»; G-code своего движка с `;TYPE:`/M73; тур NOZZA печатает настоящую нарезку |
| 18.11 | Пульт станка: регулятор скорости, тракт AMS, шкала слоёв G-code, шторка инцидентов, звуки панели; шлюз Studio без «код=-1» от молчунов и ретранслятор станков для другой подсети |
...
| 18.4 | Хотелки очередью и кастом без товара в базе: захват лично и из чата, статусы «нашёл → в заказе», клонирование позиций, повтор из дела клиента |
...
| 18.12.4 | Флаер кофейням: визитка-флаер A6 к коробке с образцами — блоки 01–04, QR в Telegram, именные флаеры по списку заведений, сгиб или двусторонняя печать |
| 18.12.3 | Бот отвечает текстом и картинками без Mini App: сводка, принтеры с температурами, заказы, полка, очередь, кадр и касса в чате; починены деньги (таблица `money_log`) и полка (таблица `categories`) |
...
├── ПРОМТ-ГРИЛЛ.md                  # допрос по плану до кода: вопросы по одному, к каждому — рекомендуемый ответ, финал — план
...
├── agent/                          # личный ассистент компьютера: реестр навыков, своя база, индекс документов, окно (свои зависимости)
├── connector/
...
│   ├── assistant.html              # помощник: разговор, каталог действий, голос, журнал
...
- **Флаер кофейням** — визитка-флаер A6 105 × 148 (или 95 × 135) к коробке с тестовыми образцами: лицо с QR в Telegram, оборот с блоками 01–04, список заведений печатает каждому флаеру своё имя. Сгиб пополам или двусторонняя печать, три темы.
