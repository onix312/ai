...
const AUTO_EXTRA = [
  ...
  ['ams_remain_tolerance', 'Расхождение AMS и склада, %', 'Выше порога остаток не перетирается сам', 'num', 1],
  ['ams_remain_confirm', 'Спрашивать при расхождении AMS', 'Остаток записывается только кнопкой «Принять AMS»', 'bool'],
  ['ams_map_prefer_known', 'Выбирать слот по катушке', 'Катушка из базы важнее похожого оттенка', 'bool'],
  ['spool_revalue_on_restock', 'Переоценивать катушку при приёме', 'Цена грамма — по средневзвешенной', 'bool'],
  ['filament_diameter_mm', 'Диаметр прутка, мм', 'Для пересчёта метров слайсера в граммы', 'num', 0.05],
