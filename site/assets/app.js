...
// ================================================================
// БАЗА ИЗДЕЛИЙ — стартовые гипотезы трёх ниш (обновлено в 2026)
// [категория, название, вес г, часы, цена ₽, отход AMS г, шт на стол]
// ================================================================
const PRESETS=[
 // Стартовые оценки — замените вес, время и цену данными своего слайсера и рынка.
 ['🐾 Питомцы','Адресник с именем и телефоном (PETG, 2 цвета)',8,0.55,450,3,16],
 ['🐾 Питомцы','Именная табличка зоны питомца',30,1.4,650,7,4],
 ['🐾 Питомцы','Держатель поводка настенный',75,3.2,950,4,2],
 ['🐾 Питомцы','Диспенсер пакетов',35,1.7,550,0,4],
 ['🐾 Питомцы','Органайзер аксессуаров',120,5.0,1400,0,1],
 ['🐾 Питомцы','Силуэт породы / памятная рамка',55,2.4,850,5,2],
 ['🐾 Питомцы','Крепление для клетки по размерам',35,1.6,750,0,4],
 ['🏠 Для дома','Держатель кабелей, комплект',25,1.1,450,0,8],
 ['🏠 Для дома','Держатель пульта по размеру',45,2.0,650,0,4],
 ['🏠 Для дома','Крепление роутера',95,4.0,1200,0,2],
 ['🏠 Для дома','Разделители ящика, комплект',140,5.5,1500,0,1],
 ['🏠 Для дома','Органайзер рабочего стола',110,4.5,1300,0,1],
 ['🏠 Для дома','Настенный карман',90,3.8,1100,0,2],
 ['🏠 Для дома','Держатель инструмента',80,3.2,950,0,2],
 ['🏠 Для дома','Некритичная ручка или заглушка',25,1.2,550,0,6],
 ['🏪 Для бизнеса','QR-стойка со сменной вставкой',45,1.9,850,5,4],
 ['🏪 Для бизнеса','Держатель ценника',10,0.45,250,0,12],
 ['🏪 Для бизнеса','Настольная табличка (AMS, 2 цвета)',35,1.6,750,7,4],
 ['🏪 Для бизнеса','Номерок с нумерацией',12,0.55,300,3,12],
 ['🏪 Для бизнеса','Органайзер кассы по размерам',150,6.0,1800,0,1],
 ['🏪 Для бизнеса','Трафарет по макету',18,0.8,550,0,8],
 ['🏪 Для бизнеса','Декоративный топпер',8,0.45,450,3,8],
 ['🛠 Услуги','Печать по готовому файлу (пример заказа)',100,4.0,1400,0,1],
 ['🛠 Услуги','Прототип с простой адаптацией модели',80,3.5,2200,0,1]
];
...
// ================================================================
const CHANNELS={
 shop:  {n:'Магазин / полка — проверьте налог и кассу', fee:0, fix:0},
 tg:    {n:'Прямая продажа — внесите эквайринг',       fee:0, fix:0},
 avito: {n:'Авито — внесите текущую комиссию',         fee:0, fix:0},
 wb:    {n:'Wildberries — внесите тариф кабинета',     fee:0, fix:0},
 ozon:  {n:'Ozon — внесите тариф кабинета',            fee:0, fix:0}
};
...
let CATALOG=(()=>{
 const d=store(CAT_KEY,null);
 if(!Array.isArray(d)||!d.length) return catDefaults();
 // Автоматически обновляем только нетронутую стартовую базу старой версии.
 // Пользовательские и отредактированные каталоги не перезаписываем.
 const untouchedLegacy=d.length===25&&d[0]?.n==='Дракон кристальный 18 см'&&d[24]?.n==='Подарочный набор (дракон + яйцо + коробка)';
 if(untouchedLegacy){const fresh=catDefaults();save(CAT_KEY,fresh);return fresh;}
 return catMigrate(d);
})();
...
// ================================================================
// ПАРТИОННЫЙ РАСЧЁТ — несколько одинаковых изделий на столе
...
function backupData(){
 const operations={};
 ['ops_orders1','ops_customers1','ops_statuses1','ops_niches1','ops_order_view'].forEach(k=>{
  const v=localStorage.getItem(k); if(v!==null) operations[k]=v;
 });
 return {app:'3dprint-guide',v:3,date:new Date().toISOString(),
  catalog:CATALOG,sales:SALES,history:HIST,plan:PLAN,spools:SPOOLS,
  operations,fields:BK_FIELDS(),autochk:localStorage.getItem('sp_autochk')||'0'};
}
...
function doRestore(file){
 ...
 fr.onload=()=>{
  ...
  if(d.operations&&typeof d.operations==='object'){
   ['ops_orders1','ops_customers1','ops_statuses1','ops_niches1','ops_order_view'].forEach(k=>localStorage.removeItem(k));
   Object.keys(d.operations).forEach(k=>localStorage.setItem(k,d.operations[k]));
  }
...
function bkStatus(){
 ...
 const days=last?Math.floor((Date.now()-new Date(last).getTime())/864e5):999;
  const has=CATALOG.length!==PRESETS.length||HIST.length>0||Object.keys(SALES).length>0||!!localStorage.getItem('ops_orders1');
 if(rem){
