...
Что делает:
  * собирает шесть «карточек-чатов» в стиле Telegram (светлая тема):
...
Запуск из корня репозитория:: python3 scripts/bot-ideas-svg.py
"""
...
def poster() -> str:
    """Сетка 2×3: заголовок бренда, карточки, подвал."""
    gap = 26
    margin = 28
    cols = 2
    ...
    out = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}"'
           ...
           text(margin, 82, "Бот сотрудников: следующие шаги", 26, "#FFFFFF",
                True),
           text(margin, 108, "шесть макетов к docs/ИДЕИ-БОТ-18.2.md · "
