#!/usr/bin/env python3
"""Живая приёмка пульта цеха (18.0.5): семь сценариев ТЗ на копии базы.
...
проходит те же семь сценариев по серверной части: страница и сводка, команды с
...
def untouched(probe: Probe, source: Path, stamps: dict[str, float]) -> None:
    ...
    probe.check("7. рабочая база не тронута", check)
...
def run_checks(probe: Probe, client: Client, copied: bool) -> None:
    ...
    if not copied:
        probe.check("7. рабочая база не тронута",
