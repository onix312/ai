"""Реестр навыков личного ассистента (18.14): навык — это данные, а не ветка кода.

Зачем реестр, если можно написать `if name == "files.search"`.

Ассистент компьютера растёт навыками: сегодня их семнадцать, в каталоге
`docs/НАВЫКИ.md` их пятьдесят семь. Если каждый навык добавлять веткой в
диспетчер, то к тридцатому навыку подтверждение, журнал и проверка параметров
разъедутся по трём местам — ровно так, как разошлись `api.py` и `routes_*.py`.
Поэтому здесь одно описание навыка, из которого берутся:
...
# ---------------------------------------------------------------------------
# Реестр: семнадцать живых навыков из каталога `docs/НАВЫКИ.md`
# ---------------------------------------------------------------------------
#
