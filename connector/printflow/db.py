...
ADDED_COLUMNS: dict[str, list[tuple[str, str]]] = {
    ...
    "nom_variants": [
        ...
        ("photo", "TEXT DEFAULT ''"),
        # 18.6: вариация — полноценный товар. Бренд и описание — как у старшей
        # карточки; галерея — JSON-список файлов сверх обложки (сама обложка
        # остаётся в photo, чтобы старые раздачи отдавали те же байты).
        ("brand", "TEXT DEFAULT ''"),
    ...
    "spec_items": [
        # 17.0.14: склад-источник расходника. Раньше состав списывался только
        # со склада изделия — расходники с отдельного склада («Материалы»,
    ...
    "cashier_tokens": [
        ...
        # по `expires_at` нельзя отличить «касса на смене» от «касса ушла в
        # офлайн», а без этого владелец узнаёт об обрыве только от кассира.
    ...
    "cashier_sales": [
        ...
        ("box_id", "TEXT DEFAULT ''"),
        # 17.0.6: возврат товара покупателем (не сторно!). Продажа остаётся в
        # журнале и в своей смене, а деньги уходят возвратной проводкой
        # сегодня; refund_items хранит уже возвращённое по строкам, чтобы
        # можно было вернуть часть, а потом остаток.
        ("refunded_at", "TEXT DEFAULT ''"),
        ("refunded_by", "TEXT DEFAULT ''"),
        ("refunded_amount", "REAL DEFAULT 0"),
        ("refund_items", "TEXT DEFAULT '[]'"),
        ("refund_request_id", "TEXT DEFAULT ''"),
        # 17.0.8 «офлайн-контур»: время продажи на кассе (она могла быть
        # совершена во время обрыва и приехать только что) и флаги выгрузки —
        # минус-остаток, неполный склад. Журнал пишется по факту выгрузки,
        # поэтому без этих двух полей «продажа 12:03» и «проводка 12:41»
        # выглядели бы подделкой.
        ("offline_at", "TEXT DEFAULT ''"),
        ("offline_flags", "TEXT DEFAULT ''"),
    ],
    "cashier_shifts": [
    ...
    "shelf_moves": [
        ...
        # Отмена продажи: 1 — движение аннулировано, штуки возвращены на стеллаж,
        # связанная финансовая проводка удалена. Движение остаётся в журнале для аудита.
        ("undone", "INTEGER DEFAULT 0"),
        # 17.0.6: сколько штук по этому движению уже вернули (частичные возвраты
        # не должны позволять вернуть больше, чем продали по строке).
