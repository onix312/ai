...
class Npd:
    ...
    def day_income(self, day: Any = "") -> dict:
        ...
        row = self.db.one(
            ...
            (d.isoformat(),)) or {}
        # Возврат — тоже расчёт: по нему выдаётся чек возврата. Если день
        # выглядит «без денег», потому что всё вернули, он всё равно обязан
        # попасть в очередь подтверждения, иначе чек возврата никто не спросит.
        back = self.db.one(
            "SELECT COALESCE(SUM(amount),0) AS total, COUNT(*) AS docs FROM transactions"
            " WHERE kind='expense' AND category='refund' AND date(at)=date(?)",
            (d.isoformat(),)) or {}
        return {"day": d.isoformat(), "income": round(num(row.get("total")), 2),
                "company": round(num(row.get("company")), 2),
                "docs": _int(row.get("docs")),
                "refunds": round(num(back.get("total")), 2),
                "refund_docs": _int(back.get("docs")),
                # оборот дня: и взятое, и возвращённое — по нему решаем, нужен
                # ли вообще чек (чек возврата — тоже чек)
                "turnover": round(num(row.get("total")) + num(back.get("total")), 2)}
    ...
    def days(self, back: int = 14) -> list[dict]:
        ...
        for offset in range(max(1, _int(back) or 14)):
            ...
            data["overdue"] = bool(data["turnover"]) and not mark \
                and day < today
            out.append(data)
        return out
    ...
    def pending(self, lookback: int = 31) -> dict:
        ...
        # Штраф считается от суммы расчёта без чека; возврат — тоже расчёт,
        # поэтому берём оборот дня, а не только взятые деньги.
        total = round(sum(num(r.get("turnover", r["income"])) for r in need), 2)
        return {"days": len(need), "amount": total,
                "income": round(sum(num(r["income"]) for r in need), 2),
                "refunds": round(sum(num(r.get("refunds")) for r in need), 2),
                "list": need[:10],
                "fine_min": round(total * 0.2, 2),
                "fine_max": total}
