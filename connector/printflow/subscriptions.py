...
EVENTS: dict[str, tuple[str, str]] = {
    ...
    "cashier_offline": ("Касса ушла в офлайн", "Касса"),
    "backup_failed": ("Бэкап не выполнен", "Система"),
    "diagnostics_alert": ("Самодиагностика: проблема", "Система"),
    # 18.13: автопилот AMS пишет в Telegram только про важное — отвязку катушки,
    # потерю пластика, конфликт «в слоте не то» и кончившийся на печати пластик.
    # Каждое — отдельное событие: мастер по печати может отписаться от остального,
    # но авария на печати (`ams_runout`, критичное) придёт всем в общий чат.
    "ams_unbind": ("AMS: катушка снята со слота", "Материалы"),
    "ams_empty": ("AMS: катушка кончилась в слоте", "Материалы"),
    "ams_loss": ("AMS: потеря пластика", "Материалы"),
    "ams_conflict": ("AMS: в слоте не тот пластик", "Материалы"),
    "ams_runout": ("AMS: пластик кончился на печати", "Печать"),
}
...
DEFAULT_ON = frozenset({
    ...
    # Важное от автопилота AMS: это и есть договорённость «только про важное».
    "ams_unbind", "ams_empty", "ams_loss", "ams_conflict", "ams_runout",
})
