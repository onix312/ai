...
META.update({
    ...
    "ams_autopilot": ("printers", "AMS: автопилот (привязки, переносы, журнал)", {}),
    "ams_adopt_generic": ("printers", "AMS: узнавать сторонний пластик без RFID", {}),
    "ams_push_settings": ("printers", "AMS: писать настройки слота в принтер", {}),
    "ams_push_retry_min": ("printers", "AMS: повтор записи в слот, мин", {"min": 1, "max": 1440}),
    "ams_learn": ("printers", "AMS: учиться на правках катушек", {}),
    "notify_ams_auto": ("printers", "AMS: важное от автопилота в Telegram", {}),
...
HINTS: dict[str, str] = {
    ...
    "ams_autopilot": "Автопилот AMS: сам привязывает и переносит катушки, разбирает пустые и остатки, ведёт журнал действий с откатом.",
    "ams_adopt_generic": "Сторонняя катушка без RFID узнаётся по недавней истории слотов и цвету — только если кандидат один.",
    "ams_push_settings": "Приводить тип, цвет и температуры слота в принтере к складской катушке. Во время печати настройки не трогаются.",
    "ams_push_retry_min": "Не отправлять один и тот же набор настроек слота чаще, чем раз в столько минут.",
    "ams_learn": "Запоминать правки карточек катушек: после двух одинаковых правок значение подставляется само (см. правила AMS).",
    "ams_material_defaults": "Таблица «материал → масса, цена, бренд» для новых катушек из AMS. Правится на вкладке «Принтеры» → AMS-доктор.",
    "notify_ams_auto": "Сообщать в Telegram только о важном: отвязка катушки, потеря пластика, конфликт слота, пластик кончился на печати.",
