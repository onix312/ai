...
@router.get("/api/farmloop/ai-detect", doc="AI детектор по камере")
def farmloop_ai_detect(api: Any, ctx: Ctx):
    pid = str(ctx.one("printer_id", "") or "")
    manager = getattr(api, "manager", None)
    if not manager:
        return {"ok": False, "error": "менеджер недоступен"}
    printer = manager.get(pid) if pid else manager.get("")
    if not printer:
        return {"ok": False, "error": "принтер не найден"}
    try:
        frame = getattr(printer.camera, "frame", None)
        if not frame:
            return {"ok": False, "error": "нет кадра камеры"}
        from .config import PHOTO_DIR
        ref_path = PHOTO_DIR / "bed_reference.jpg"
        ref = ref_path.read_bytes() if ref_path.exists() else None
        from .spaghetti import frame_diff_ratio_advanced
        roi = None
        try:
            import json as _json
            raw = api.db.setting("bed_roi", "")
            ...
            roi = None
        result = frame_diff_ratio_advanced(frame, ref or frame, roi=roi)
        # twin info
        twin = {}
        if hasattr(manager, "farmloop_digital_twin"):
            twin = manager.farmloop_digital_twin(pid)
        return {"ok": True, "ai": result, "twin": twin, "printer_id": printer.id}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}


@router.get("/api/farmloop/force", doc="Force feedback толкателя")
def farmloop_force(api: Any, ctx: Ctx):
    pid = str(ctx.one("printer_id", "") or "")
    manager = getattr(api, "manager", None)
    if not manager:
        return {"ok": False, "error": "менеджер недоступен"}
    try:
        push_mm = float(api.db.setting(f"farmloop_push_mm_{pid}", 15.0) or 15.0) if pid else 15.0
        # auto-tune history
        tune = getattr(manager, "_farmloop_push_tune", {}).get(pid, push_mm) if pid else push_mm
        metrics = manager.farmloop_metrics() if hasattr(manager, "farmloop_metrics") else {}
        return {"ok": True, "printer_id": pid, "push_mm": push_mm, "tuned": tune, "metrics": metrics, "force_estimate": round(tune * 0.5, 1)}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}


@router.post("/api/farmloop/force", audit="Конвейер: force", doc="Сохранить force feedback")
def farmloop_force_post(api: Any, ctx: Ctx):
    body = ctx.body or {}
    pid = str(body.get("printer_id") or "").strip()
    push_mm = body.get("push_mm")
    if push_mm is None:
        return 400, {"error": "push_mm required"}
    try:
        val = float(push_mm)
        if not (5 <= val <= 60):
            return 400, {"error": "push_mm 5..60"}
        if pid:
            api.db.set_setting(f"farmloop_push_mm_{pid}", val)
        else:
            api.db.set_setting("farmloop_push_mm_default", val)
        return {"ok": True, "push_mm": val, "printer_id": pid}
    except Exception as exc:
        return 400, {"error": str(exc)}


@router.post("/api/farmloop/bed/reference", audit="Конвейер: эталон стола", doc="Сохранить эталон пустого стола")
def farmloop_bed_reference_post(api: Any, ctx: Ctx):
    body = ctx.body or {}
    pid = str(body.get("printer_id") or "").strip()
    manager = getattr(api, "manager", None)
    if not manager:
        return {"ok": False, "error": "менеджер недоступен"}
    try:
        res = manager.set_bed_reference(pid) if pid else manager.set_bed_reference(next(iter(getattr(manager, "printers", {})), ""))
        return res
    except Exception as exc:
        return 400, {"error": str(exc)}


@router.get("/api/farmloop/ams/mappings", doc="Список сохранённых AMS маппингов")
def farmloop_ams_mappings(api: Any, ctx: Ctx):
    try:
        from .spool_mapping_repo import list_mappings
        rows = list_mappings(api.db, limit=int(ctx.one("limit", "50") or "50"))
        return {"ok": True, "mappings": rows}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}


@router.post("/api/farmloop/ams/mappings", audit="AMS: save mapping", doc="Сохранить AMS маппинг")
def farmloop_ams_mappings_post(api: Any, ctx: Ctx):
    body = ctx.body or {}
    file_hash = str(body.get("file_hash") or body.get("hash") or "").strip()
    filename = str(body.get("filename") or body.get("file") or "").strip()
    printer_id = str(body.get("printer_id") or "").strip()
    mapping = body.get("mapping") or body.get("ams_mapping") or []
    if not file_hash and not filename:
        return 400, {"error": "file_hash or filename required"}
    # if no hash, compute from filename if exists
    if not file_hash and filename:
        try:
            ...
            pass
    if not file_hash:
        file_hash = filename.lower()
    try:
        from .spool_mapping_repo import save_mapping
        res = save_mapping(api.db, file_hash, filename, printer_id, mapping if isinstance(mapping, list) else [])
        return res
    except Exception as exc:
        return 400, {"error": str(exc)}


@router.get("/api/farmloop/accounting/report", doc="Отчёт экономики конвейера")
def farmloop_accounting_report(api: Any, ctx: Ctx):
    try:
        from .accounting import Accounting
        acc = Accounting(api.db)
        days = int(ctx.one("days", "7") or "7")
        # reuse existing stats
        summary = acc.summary(days=days) if hasattr(acc, "summary") else {}
        # farmloop metrics
        manager = getattr(api, "manager", None)
        farm = manager.farmloop_metrics() if manager and hasattr(manager, "farmloop_metrics") else {}
        # queue balance
        balance = manager.balance_queue() if manager and hasattr(manager, "balance_queue") else {}
        return {"ok": True, "days": days, "accounting": summary, "farmloop": farm, "balance": balance}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}


@router.get("/api/farmloop/bed/overlay", doc="Overlay проекции на камере")
def farmloop_bed_overlay(api: Any, ctx: Ctx):
    pid = str(ctx.one("printer_id", "") or "")
    manager = getattr(api, "manager", None)
    if not manager:
        return {"ok": False, "error": "менеджер недоступен"}
    ...
    try:
        # projection + roi
        import json as _json
        proj_raw = api.db.setting("bed_projection", "")
        proj = _json.loads(proj_raw) if isinstance(proj_raw, str) and proj_raw else proj_raw
        roi_raw = api.db.setting("bed_roi", "")
        roi = _json.loads(roi_raw) if isinstance(roi_raw, str) and roi_raw else roi_raw
        # preview path from timeline
        from pathlib import Path
        from .config import DATA_DIR
        profile = str(ctx.one("profile", P1S_STAGE1.id) or P1S_STAGE1.id)
        tl_path = DATA_DIR / "farmloop-templates" / f"{profile}.timeline.json"
        preview_path = []
        if tl_path.is_file():
            try:
                blocks = _json.loads(tl_path.read_text(encoding="utf-8"))
                from .farmloop import build_from_timeline
                built = build_from_timeline(blocks)
                preview_path = built.get("preview_path", [])
            except Exception:
                preview_path = []
        return {"ok": True, "projection": proj, "roi": roi, "preview_path": preview_path, "printer_id": printer.id}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}
