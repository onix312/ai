...
DEFAULT_SETTINGS: dict[str, object] = {
    ...
    # Сверка остатка AMS с базой (идея 21): выше порога — не перетираем граммы
    # процентами, а спрашиваем оператора.
    "ams_remain_tolerance": 25,   # расхождение в процентных пунктах
    "ams_remain_confirm": True,   # требовать подтверждение при расхождении
    # Автоподбор слотов AMS (идея 20): катушка, известная базе по RFID-метке,
    # важнее «просто похожего оттенка».
    "ams_map_prefer_known": True,
    # Переоценка катушки по средневзвешенной цене при приёме (идея 30).
    "spool_revalue_on_restock": True,
    # Диаметр прутка для пересчёта метров в граммы (идея 26).
