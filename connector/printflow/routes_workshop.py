...
@router.post("/api/jobs/spool", audit="Катушка задания печати",
             doc="Катушка задания: чем печатается эта плита (17.0.26)")
def job_spool_save(api: Any, ctx: Ctx):
    """Выбрать катушку задания в очереди — или снять выбор.

    Раньше катушку можно было указать только в модалке запуска файла: задание
    из заказа оставалось без неё, и к завершению пластик списывался с той
    катушки, что стоит в активном слоте AMS. Переставили катушки — расход ушёл
    на чужой материал. Здесь выбор живёт на самом задании, а пустой `spool_id`
    честно означает прежнее поведение: «определить по AMS».
    """
    body = ctx.body or {}
    return api.manager.set_job_spool(str(body.get("id") or ""),
