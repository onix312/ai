"""Тексты и картинки бота — цех словами прямо в чате.

Зачем. Чат читается из очереди к станку, на ходу и с любого телефона —
ему не нужны внешние адреса и отдельные окна. Бот отвечает сам: текстом
и, когда есть что показать, фотографией (кадр камеры принтера или снимок
изделия из фотоальбома заказа).
...
def camera_absent_text(printers: list[dict]) -> str:
    ...
    return ("📷 Кадр недоступен: камера не отдала ни одного кадра"
            ...
            + ".\nПроверьте в панели, что камера станка включена.")
