...
* **Строковые контракты**: разметка шапки и шторки в `index.html`, пины
  `?v=18.13.0`, иконки реестра, исправление `p.temperature` в Hero-пульте,
...
class MarkupContractTests(unittest.TestCase):
    ...
    def test_new_assets_are_wired_with_current_version(self):
        for name in NEW_ASSETS:
            self.assertIn(f'<script src="assets/{name}?v=18.13.0"></script>', INDEX, name)
            self.assertIn(f"'/assets/{name}'", SW_JS, f"{name} должен быть в SHELL sw.js")
        self.assertIn('<link rel="stylesheet" href="assets/controls.css?v=18.13.0">', INDEX)
        self.assertIn("'/assets/controls.css'", SW_JS)
        self.assertIn('<script src="assets/app.js?v=18.13.0"></script>', INDEX)
        self.assertIn('<script src="assets/core.js?v=18.13.0"></script>', INDEX)
        self.assertIn("printflow-shell-v82", SW_JS)
    ...
    def test_gpuslice_asset_wired(self):
        self.assertIn('<script src="assets/gpuslice.js?v=18.13.0"></script>', INDEX)
