 ...
 *
 * Адрес сервера в LAN — единственное, что в пульте реально способно всё сломать:
 * роутер выдал компьютеру новый IP, и телефон у станка «не видит цех». Поэтому
 * здесь две вещи: короткая проверка сохранённого адреса (1,6 с — оператор не
 * должен ждать) и поиск по своей /24 (по 0,7 с на адрес, общий потолок ~4 с).
 *
 * Порты перебираем начиная с 8790 — это порт коннектора по умолчанию; 8765 и
 * 8080 оставлены для установок, где порт меняли при запуске.
 */
object Net {

    val PORTS = intArrayOf(8790, 8765, 8080)
    ...
    private fun netKey(base: String): String {
        ...
        return host.split('.').joinToString(".") {
            (it.toIntOrNull() ?: 0).toString().padStart(3, '0')
        }
    }
}
